    ...
    public static treeToQuad(data: Buffer, path: string): Quad[] {
        ...
        for (let i = 0; i < oidArray.length; i++) {
            if (modeArray[i] === "0644") {
                ...
                let pathToNewTree= path.slice(0,path.indexOf("objects")+8)+oidArray[i].slice(0,2)+"/"+oidArray[i].slice(2);
                const t = triple(namedNode(fileNamesArray[i]), namedNode(ldp.LDP.NonRDFSource), namedNode(pathToNewTree));
                quads.push(t);
            } else if (modeArray[i] === "0000") {
                let pathToNewTree= path.slice(0,path.indexOf("objects")+8)+oidArray[i].slice(0,2)+"/"+oidArray[i].slice(2);
                const t = triple(namedNode(fileNamesArray[i]), namedNode(ldp.LDP.BasicContainer), namedNode(pathToNewTree));
                quads.push(t);
